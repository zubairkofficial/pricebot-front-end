...
function InvoiceDetail() {
    ...
    return (
        <div className="container mt-5" style={{ position: 'relative' }}>
            <Link to={'/Records'} className="btn btn-secondary position-absolute top-0 end-0">Zurück</Link>
                    ...
                    {isLoading ? (
                        ...
                        <div className="row row-cols-1 row-cols-md-3 g-4">
                            {invoiceData.map((invoice, index) => (
                                <div className="col" key={index} onClick={() => openModal(invoice)} style={{ cursor: 'pointer' }}>
                                    <div className={`card h-100 ${invoice.matched ? 'border-success' : 'border-warning'}`}>
                                        <div className="card-header bg-primary text-white">
                                            <h4 className="text-center mb-0">Rechnung {index + 1}</h4>
                                        </div>

                                        <div className="card-body">
                                            <p><strong>Titel:</strong> {invoice.title}</p>
                ...
                <div className="modal-dialog modal-dialog-centered">
                    <div className="modal-content">
                        <div className="modal-header">
                            ...
                            <button type="button" className="btn-close" data-bs-dismiss="modal" aria-label="Schließen" onClick={closeModal}></button>
                        </div>
