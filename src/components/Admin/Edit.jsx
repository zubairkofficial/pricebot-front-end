...
const EditUserForm = () => {
  ...
  useEffect(() => {
    const fetchUserDetails = async () => {
      try {
        ...
        setUser({
          name: data.name,
          services: data.services.map(serviceName => myData.find(service => service.value === serviceName)),
        });
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/editRole/${roleId}`, {
        method: 'PUT',
        headers: {
      ...
      navigate('/Admin', { state: { alertMessage: 'User updated successfully' } });
    } catch (error) {
        ...
        <div className="col-md-10">
          <div className=" p-4 rounded-3 shadow " style={{backgroundColor:'transparent'}}>
            <form onSubmit={handleSubmit}>
              ...
              <div className="mb-3">
                <label htmlFor="serviceSelect" className="form-label">Services</label>
                <Select
                  options={myData}
              ...
              <div className="d-flex justify-content-start">
                <button type="submit" className="btn btn-outline-primary">Benutzer aktualisieren</button>
                <Link to={`/Admin`} className="btn btn-outline-danger ms-2">Stornieren</Link>
              </div>
            </form>
          </div>
        </div>
